...
Std_ReturnType seven_segment_initialize(const segment_t *seg){
  Std_ReturnType ret = E_OK;
    if(NULL == seg){
        ...
        ret = gpio_pin_initalize(&(seg->segment_pins[SEGMENT_PIN0]));
        ret = gpio_pin_initalize(&(seg->segment_pins[SEGMENT_PIN1]));
        ret = gpio_pin_initalize(&(seg->segment_pins[SEGMENT_PIN2]));
        ret = gpio_pin_initalize(&(seg->segment_pins[SEGMENT_PIN3]));
    }
    return ret;  
}
